...
export const getNewAccessToken = async (token?: string) => {
    ...
    const refreshToken = token || cookieStore.get("refreshToken")?.value || null;
...
export const isAccessTokenExist = async () => {
    ...
    const decodedAccessToken = accessToken ? await jwtUtils.verifyToken(accessToken, process.env.JWT_ACCESS_SECRET as string) : null;
